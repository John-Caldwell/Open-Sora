...
batch_size = 1
dtype = "fp16"  # Using fp16 for better compatibility
offload_model = True  # Enable CPU offloading to save VRAM
...
sampling_option = dict(
    ...
    num_frames=33,  # Reduced to 33 frames to fit in 8GB VRAM (~1.4 seconds)
    num_steps=30,  # Reduced from 50 for faster generation (~40% faster)
    shift=True,
...
model = dict(
    ...
    strict_load=False,  # Allow partial loading for checkpoint mismatch
...
ae = dict(
    type="hunyuan_vae",
    from_pretrained="./ckpts/hunyuan_vae.pt",
    in_channels=3,
    out_channels=3,
    layers_per_block=2,
    latent_channels=16,
    use_spatial_tiling=True,
    use_temporal_tiling=True,  # Enable temporal tiling to reduce memory
)
t5 = dict(
    type="text_embedder",
